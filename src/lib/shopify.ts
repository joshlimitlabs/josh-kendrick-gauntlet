export function addToCartViaRedirect(
  variantId: string,
  ...
  const params = new URLSearchParams({
    id: numericVariantId,
    quantity: '1',
    return_to: 'cart' // redirect back to cart page
  });
  
  // add custom attributes as properties
  customAttributes.forEach(attr => {
    params.append(`properties[${attr.key}]`, attr.value);
  });
  
  const cartUrl = `https://limitlabs.com/cart/add?${params.toString()}`;

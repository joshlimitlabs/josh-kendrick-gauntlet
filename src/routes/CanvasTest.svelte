<script lang='ts'>
  ...
  const canvasScaleBase = 3;
	const canvasScale = 2;
  // const canvasScaleRatio = canvasScale/canvasScaleBase;
  const canvasScaleRatio = 1;
  ...
  function canvasRedraw(drawScale: number, drawWidth: number, drawColor: string, exporting: boolean){
    ...
    if(!exporting && (!visibleLayers[0] || selectedDevice == ''))
    {
      ...
      else
      {
        ...
        let pageBleedWidth = canvasScaleRatio*(deviceFullWidth+136)/pageBleedScale; //136 instead of 150 to account for rounding error in multiplication
        let pageBleedHeight = canvasScaleRatio*(deviceFullHeight+136)/pageBleedScale;
    ...
    rectangleCtx?.save();
    rectangleCtx?.scale(2/3, 2/3);
    rectangleCtx?.restore();
    imageCtx?.save();
    imageCtx?.scale(2/3, 2/3);
    imageCtx?.restore();
  }
    ...
    if (!deviceDrawn)
    {
      ...
      img.onload = () => {
        ...
        if (rectangleCanvas && rectangleCtx)
        {
          ...
          {
            const x = (rectangleCanvas.width - img.width*canvasScaleRatio) / 2;
            const y = (rectangleCanvas.height - img.height*canvasScaleRatio) / 2;
            rectangleCtx.scale(canvasScaleRatio, canvasScaleRatio);
